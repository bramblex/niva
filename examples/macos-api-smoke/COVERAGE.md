...
The 164 rows below are the distinct Native case IDs actually tracked in this table. A prior 167/167 aggregate report cannot be reconciled to these 164 rows and remains historical only; no three missing IDs are inferred. Fixtures have since been migrated away from `Niva.api`: native methods use direct Niva namespaces, Node behavior uses Node modules, and `run_all.py` carries a one-to-one crosswalk for renamed cases. Dated PASS entries are historical evidence from 2026-09-24; migrated cases have not yet been rerun on a real macOS WebView. Case definitions and old PASS rows do not count as current results. Picker cancellation and selection remain separate branches.
...
| `host.send` | Historical case ID superseded by the fixture-owned JSON process protocol: send a unique nonce through fixture process command and require an exact process-stream echo. | default/headless-safe fixture process-stream echo | historical PASS real macOS: default; migrated rerun pending |
...
| `process.args` | Require this run’s isolated `--config`/`--resource` paths in `process.argv`. | headless-safe run.py / headless.html | historical PASS real macOS: headless; migrated rerun pending |
...
| `windowExtra.setEnable` | On macOS call `Niva.windowExtra.setEnable(false, 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.setTaskbarIcon` | On macOS call `Niva.windowExtra.setTaskbarIcon("niva-smoke-unique", 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.theme` | On macOS call `Niva.windowExtra.theme(0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.resetDeadKeys` | On macOS call `Niva.windowExtra.resetDeadKeys(0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.beginResizeDrag` | On macOS call `Niva.windowExtra.beginResizeDrag(1, 1, 1, 1, 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.setSkipTaskbar` | On macOS call `Niva.windowExtra.setSkipTaskbar(false, 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.setUndecoratedShadow` | On macOS call `Niva.windowExtra.setUndecoratedShadow(false, 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.setOverlayIcon` | On macOS call `Niva.windowExtra.setOverlayIcon("niva-smoke-unique", 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.setRtl` | On macOS call `Niva.windowExtra.setRtl(false, 0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
| `windowExtra.hasUndecoratedShadow` | On macOS call `Niva.windowExtra.hasUndecoratedShadow(0)`; require bridge rejection code -1 and message `api not found` because this handler is registered only for Windows. | window-cases.js / expected macOS error | PASS real macOS: extended |
