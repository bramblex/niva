...
import { Buffer } from "../dist/source/buffer.js";
import net from "../dist/source/net.js";
import tls from "../dist/source/tls.js";
import dgram from "../dist/source/dgram.js";
import dns from "../dist/source/dns.js";
import dnsPromises from "../dist/source/dns-promises.js";
...
test("TLS rejects unsupported verification and identity options before transport", async () => {
  const niva = mockNiva();
  ...
  assert.equal(api.connectGuarded, undefined);
...
test("DNS uses OS lookup separately from validated DNS packet queries over UDP", async () => {
  ...
  const baseStream = niva.bridge.stream.bind(niva.bridge);
  niva.bridge.stream = (method, args, handlers) => {
