...
import nodeUtil from "../dist/source/util.js";
import nivaStream from "../dist/source/stream.js";
...
test("util.inherits establishes Node's prototype and super_ contract for Niva streams", () => {
  function SendStream() {}
  nodeUtil.inherits(SendStream, nivaStream);
  const instance = new SendStream();
  assert.equal(SendStream.super_, nivaStream);
  assert.equal(Object.getPrototypeOf(SendStream.prototype), nivaStream.prototype);
  assert.equal(instance instanceof nivaStream, true);
  assert.throws(() => nodeUtil.inherits(null, SendStream), { code: "ERR_INVALID_ARG_TYPE" });
});
