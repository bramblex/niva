...
import stream from "../dist/source/stream.js";
...
import events from "../dist/source/events.js";
import util from "../dist/source/util.js";
import assertCompat from "../dist/source/assert.js";
import path, { posix, win32 } from "../dist/source/path.js";
import querystring from "../dist/source/querystring.js";
import url from "../dist/source/url.js";
import stringDecoder from "../dist/source/string_decoder.js";
...
test("crypto hash, HMAC, random, KDF and timingSafeEqual use synchronous Node shapes", async () => {
  ...
  try {
    ...
    assert.ok(warnings.every((warning) => warning.includes("not constant-time")));
    assert.ok(warnings.every((warning) => !warning.includes("1,2") && !warning.includes("1,3")));
  } finally {
...
test("path glob syntax, dynamic cwd and file URL platform options match Node", () => {
  ...
  let cwd = "/first";
  globalThis.Niva = { bridge: { callSync(method, args) { assert.equal(method, "process.currentDir"); assert.deepEqual(args, []); return cwd; } } };
  runtime.callSync = (niva, method, args) => niva.bridge.callSync(method, args);
  try {
