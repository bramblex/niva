(function (root: any) {
  ...
  function createNetModule(niva?) {
    ...
    function weakSocketHandlers(reference) {
      return {
        onEvent: function (name, data) { var socket = reference.deref(); if (socket) socket._onBridgeEvent(name, data); },
        onChunk: function (chunk, stderr) { var socket = reference.deref(); if (socket && !stderr) socket._onBridgeChunk(chunk); },
      };
    }
    ...
    function weakServerHandlers(reference) {
      return { onEvent: function (name, data) { var server = reference.deref(); if (server) server._onListenerEvent(name, data); } };
    }
    ...
    class Socket extends Duplex {
      ...
      _start(method, args, readyEvent, routeOwner?) {
        ...
        var weak = typeof WeakRef === "function" ? new WeakRef(this) : null;
        try {
          var handlers = weak ? weakSocketHandlers(weak) : {
        ...
        if (weak) {
          this._bridgeCall.promise.then(function () {
            ...
            if (socket && !socket._remoteClosed) socket._onBridgeClose(false);
          }, function (error) {
            var socket = weak.deref();
            if (!socket) return;
            socket._hadError = true;
            socket.destroy(runtime.nativeError(error));
          });
        } else {
          var self = this;
          this._bridgeCall.promise.then(function () { if (!self._remoteClosed) self._onBridgeClose(false); }, function (error) { self._hadError = true; self.destroy(runtime.nativeError(error)); });
        }
        return this;
      }
      ...
      _onBridgeEvent(name, data) {
        ...
        if ((name === "connect" || name === "secureConnect") && data.socketId) {
          ...
          this._resourceState.socketId = data.socketId;
      ...
      _onBridgeClose(hadError) {
        if (this._remoteClosed) return;
        this._remoteClosed = true;
        if (this._resourceOwner) this._resourceOwner.release();
        this._hadError = this._hadError || hadError;
        (this as any).emit("_nativeClose", this._hadError);
        this.connecting = false;
        this.pending = false;
        this._clearTimeout();
    ...
    function parseConnectArgs(args, method = "socket.tcpConnect", eventName = "connect") {
      ...
      var nativeArgs: Record<string, any> = { host: host, port: port };
    ...
    class Server extends EventEmitter {
      constructor(options, listener, secure) {
        ...
        this.__nivaInvalidate = function (error) {
          ...
          this._socketId = undefined;
          this._address = null;
          if (this.listenerCount("error")) { try { this.emit("error", error); } catch (_) {} }
          this.emit("close");
        };
        this._address = null;
      ...
      listen() {
        ...
        if (weak) {
          this._listenerCall.promise.then(function () { var server = weak.deref(); if (server) { server.listening = false; server._maybeCloseEvent(); } }, function (error) {
            ...
            if (!server) return;
            if (!server._closing && server.listenerCount("error")) { try { server.emit("error", runtime.nativeError(error)); } catch (_) {} }
            server.listening = false;
            server._maybeCloseEvent();
          });
        } else {
          var self = this;
          this._listenerCall.promise.then(function () { self.listening = false; self._maybeCloseEvent(); }, function (error) {
            if (!self._closing && self.listenerCount("error")) { try { self.emit("error", runtime.nativeError(error)); } catch (_) {} }
    ...
    function parseListenArgs(args, secure, identity) {
      ...
      var nativeArgs: Record<string, any> = { host: options.host, port: options.port };
      if (secure) {
