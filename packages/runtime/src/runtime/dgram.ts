  ...
  function createDgramModule(niva?) {
    ...
    function weakDatagramHandlers(reference) {
      return {
        onEvent: function (name, data) {
          var socket = reference.deref();
          if (!socket) return;
          if (name === "listening") {
            socket._socketId = data.socketId;
            socket._resourceState.socketId = data.socketId;
            socket._localAddress = data.address;
            socket._localPort = data.port;
            socket._bound = true;
            socket.emit("listening");
            if (socket._closing) socket._closeNative();
          } else if (name === "datagram") socket._pendingDatagram = data;
          else if (name === "close") socket._finishClose();
        },
        onBlob: function (blob) {
          var socket = reference.deref();
          if (!socket) return;
          var info = socket._pendingDatagram || {};
          socket._pendingDatagram = undefined;
          Promise.resolve(blob.arrayBuffer()).then(function (arrayBuffer) {
            var current = reference.deref();
            if (!current) return;
    ...
    class Socket extends EventEmitter {
      ...
      bind(port, address, callback) {
        ...
        var handlers = weak ? weakDatagramHandlers(weak) : {
          onEvent: function (name, data) {
            if (name === "listening") {
        ...
        if (weak) this._bindCall.promise.then(function () { var socket = weak.deref(); if (socket) socket._finishClose(); }, function (error) {
          ...
          if (!socket) return;
          if (!socket._closing && socket.listenerCount("error")) { try { socket.emit("error", runtime.nativeError(error)); } catch (_) {} }
          socket._finishClose();
        });
        else this._bindCall.promise.then(function () { self._finishClose(); }, function (error) { if (!self._closing && self.listenerCount("error")) { try { self.emit("error", runtime.nativeError(error)); } catch (_) {} } self._finishClose(); });
        return this;
      }
