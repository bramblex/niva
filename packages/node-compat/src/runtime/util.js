(function (root) {
  ...
  function inspect(value, options) {
    ...
    function enumerableKeys(input) {
      var keys = showHidden ? Object.getOwnPropertyNames(input) : Object.keys(input);
    ...
    function visit(input, remaining, level) {
      level = level || 0;
      ...
      if (bufferObject && ArrayBuffer.isView(input)) {
        ...
        return referencePrefix + "Buffer(" + input.length + ") [Uint8Array] " + joinEntries(bufferEntries, "[", "]", level);
      }
      if (ArrayBuffer.isView(input) && !(input instanceof DataView)) {
        ...
        return referencePrefix + (constructorName(input) || "TypedArray") + "(" + input.length + ") " + joinEntries(typedEntries, "[", "]", level);
      }
      ...
      if (Array.isArray(input)) {
        ...
        if (Object.prototype.toString.call(input) === "[object KeyObject]") prefix = "SecretKeyObject [KeyObject] ";
    ...
    function formatProperty(input, key, remaining, level) {
      var descriptor = Object.getOwnPropertyDescriptor(input, key);
  ...
  function comparePartialBytes(actual, expected) {
    ...
    if (typedArrayName(actual) !== typedArrayName(expected) || expected.length > actual.length) return false;
    for (var typedIndex = 0; typedIndex < expected.length; typedIndex += 1) if (!Object.is(actual[typedIndex], expected[typedIndex])) return false;
    return true;
  }
