(function (root) {
  ...
  function nativeError(value, fallback) {
    ...
    if (value && typeof value === "object") {
      ...
      var code = /no such file|not found/.test(lower) ? "ENOENT"
  ...
  function createEmitter() {
    ...
    var emitter = {
      ...
      off: function (event, listener) {
        ...
        if (current) listeners[event] = current.filter(function (item) {
      ...
      removeAllListeners: function (event) {
        if (event === undefined) listeners = Object.create(null);
