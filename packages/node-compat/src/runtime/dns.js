(function (root) {
  ...
  function createDnsModule(niva) {
    ...
    function validatePacket(response, query, server, source) {
    ...
    function queryUdp(server, query, timeout) {
    ...
    function queryTcp(server, query, timeout) {
      return new Promise(function (resolve, reject) {
        var socket;
        var timer;
    ...
    function resolvePacket(resolver, hostname, type, seen) {
      validateHostname(hostname);
    ...
    function DnsResolver(options) { return new Resolver(options, niva); }
