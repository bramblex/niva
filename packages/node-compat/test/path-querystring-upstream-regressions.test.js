...
test("path.resolve uses the Niva main process cwd and falls back to the bridge in child windows", () => {
  ...
  try {
    assert.equal(path.posix.resolve("src", "index.js"), "/main/first/src/index.js");
    currentDir = "/main/second";
    assert.equal(path.posix.resolve("src", "index.js"), "/main/second/src/index.js");
    assert.equal(processReads, 2);
    assert.equal(nativeReads, 0);

    throwFromProcess = true;
    assert.throws(() => path.posix.resolve(), (error) => error === processError);
    assert.equal(nativeReads, 0);
    ...
    assert.equal(path.posix.resolve("src", "index.js"), "/native/first/src/index.js");
    nativeDir = "/native/second";
    assert.equal(path.posix.resolve("src", "index.js"), "/native/second/src/index.js");
    nativeDir = "";
    assert.equal(path.posix.resolve(), ".");
