...
import { totalmem as hostTotalMemory, tmpdir as hostTmpdir } from 'node:os';
...
function installOsFixture() {
  globalThis.Niva = { bootstrap: { process: { env: hostProcess.env }, os: { platform: hostProcess.platform } },
    callSync(method, parameters) {
...
async function runCase(name, synthetic) {
  ...
  try {
    ...
    if (selected?.execution === 'native') {
      ...
      globalThis.Niva = nativeFixture.bridge;
    } else installOsFixture();
    ...
    for (const id of ['buffer', 'path', 'events', 'querystring', 'string_decoder', 'crypto', 'stream', 'stream/promises', 'timers', 'timers/promises', 'url', ...(selected?.module === 'assert' ? ['assert', 'assert/strict'] : []), ...(selected?.module === 'util' ? ['util'] : []), ...(nativeFixture ? ['fs', 'fs/promises', 'os', 'process', 'net', 'dgram', 'tls', 'http', 'https', 'dns', 'dns/promises', ...(selected?.module === 'child_process' ? ['child_process'] : [])] : [])]) {
      modules[id] = (await import(pathToFileURL(path.join(root, 'src', `${id.replace('/', '-')}.js`)).href)).default;
    }
    await import(pathToFileURL(path.join(root, 'src/runtime/zlib.js')).href);
    Object.defineProperty(modules, 'zlib', {get: globalThis[Symbol.for('niva.node-compat.runtime')].createZlibModule});
...
if (args[0] === '--fixture') {
  ...
  installOsFixture();
  const nivaPath = (await import(pathToFileURL(path.join(root, 'src/path.js')).href)).default;
