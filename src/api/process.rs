...
pub async fn call(request: ApiRequest) -> ApiResponse {
    return match request.method.as_str() {
        "pid" => pid().await,
        "cwd" => cwd().await,
        "chDir" => ch_dir(request).await,
        "env" => env().await,
				"exit" => exit().await,
				"exec" => exec(request).await,
        _ => ApiResponse::err("Method not found".to_string()),
    };
}

pub async fn pid() -> ApiResponse {
...
pub async fn ch_dir(request: ApiRequest) -> ApiResponse {
...
pub async fn exec(request: ApiRequest) -> ApiResponse {
	// 执行命令
