...
- **平台验收：未完成。** 尚需 Windows 真机创建带 owner 的窗口并确认原生 owner 行为。Windows target check 不覆盖运行期行为。

### 1.2 全局快捷键管理器初始化

- **源码：已修复。** 初始化失败不再通过 `expect` 使应用启动 panic；管理器记录错误、输出诊断，快捷键能力不可用时由 API 返回错误。
- **平台验收：未完成。** 尚需在受影响平台实际验证快捷键可用与初始化失败路径。权限、占用或系统环境差异需要真机覆盖。
...
- **平台验收：未完成。** roadmap 仍要求 macOS 与 Windows 实际操作窗口菜单。源码调度路径不等于平台原生菜单行为已验收。
...
- **Windows：** 已有 `x86_64-pc-windows-msvc` target check 记录；没有 Windows 真机运行记录。owner 行为、菜单操作、托盘/图标显示及快捷键响应均未因此验收。
