...
所有页面API实现在`Niva`对象上。Node模块接口（例如`Niva.fs`）和Niva窗口接口（例如`Niva.window`）共用下列三种传输；纯JS和启动静态数据不需要第四种bridge。低层入口是`Niva.bridge`，`Niva.stream`表示Node stream模块。

## 三种传输

| 通道 | 能力 | 不支持 |
|---|---|---|
| WebSocket | 异步请求、事件、持久流、二进制；按窗口和连接隔离调用 | 未获原生授权的来源 |
| 同步XHR | 保持同步返回的Native方法、CommonJS文件解析/读取 | UI主线程操作、流、socket及未列入同步白名单的方法 |
| 平台IPC | 有界的一次性异步JSON，包含文件文本、HTTP/HTTPS文本、exec文本结果 | Native同步、ArrayBuffer/Buffer传输、持久句柄、对外流 |

WS初始连接不可用时，允许的异步操作可选择IPC；这不改变授权。已发送或已执行的调用不会因断线自动重放。无有效传输的调用明确失败，不能无限留在发送队列中。
...
二进制只走WS，头18字节：`[version=1][flags][id u64 BE][seq u64 BE]`，随后payload。flags为START=1、END=2、STDERR=4。Node HTTP/HTTPS流式协议由页面模块实现，字节走Native TCP/TLS；高层requestText则是独立的一次性Native请求。
