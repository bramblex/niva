...
本表是覆盖跟踪清单，不代表整体覆盖验收。公开 API 清单以 `packages/types/Niva_zh.d.ts` 中 `NivaObj.api` 引用的命名空间接口里声明的 `MethodSignature` 为准，共 167 个方法；不计属性，也不将类型到 handler 的名字映射视为行为覆盖。初始化脚本中的 300 次通用代理调用压力测试同样不算逐方法行为测试。直接行为测试与 helper 级测试分别标注；helper 级用例验证底层 helper，不视为完整 Niva.api handler 覆盖。原生观察仅代表所列 smoke 的实际操作。
...
| `Niva.api.dialog.pickFiles` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.dialog.pickDir` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.dialog.pickDirs` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.dialog.saveFile` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.extra.hideApplication` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.extra.showApplication` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.extra.hideOtherApplications` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.extra.setActivationPolicy` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.extra.getActiveWindowId` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
...
| `Niva.api.fs.createDir` | crates/niva/src/app/api/fs.rs — `create_dir_is_single_level_and_create_dir_all_builds_parents` (helper, not full handler) | macOS smoke: 主页面及同源 iframe 调用成功（[记录](wry-custom-protocol-plan.md#L42)） | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 Windows 原生调用观察 |
| `Niva.api.fs.createDirAll` | crates/niva/src/app/api/fs.rs — `create_dir_is_single_level_and_create_dir_all_builds_parents` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.fs.readDir` | crates/niva/src/app/api/fs.rs — `read_dir_lists_only_immediate_entry_names` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.fs.readDirAll` | crates/niva/src/app/api/fs.rs — `read_dir_all_recurses_and_excludes_matching_paths` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.host.send` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
...
| `Niva.api.os.eol` | crates/niva/src/app/api/os.rs — `separator_and_line_ending_match_the_target_platform` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.os.locale` | crates/niva/src/app/api/os.rs — `locale_uses_system_value_or_en_us_fallback` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.process.pid` | crates/niva/src/app/api/process.rs — `process_metadata_methods_return_the_current_process_values` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.process.currentDir` | crates/niva/src/app/api/process.rs — `process_metadata_methods_return_the_current_process_values` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.process.currentExe` | crates/niva/src/app/api/process.rs — `process_metadata_methods_return_the_current_process_values` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.process.env` | crates/niva/src/app/api/process.rs — `env_and_args_are_returned_as_json_map_and_ordered_array` (helper, not full handler) | 未观察 | 未观察 | 增加直达 Niva.api 的 handler 行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
...
| `Niva.api.window.setCursorPosition` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
| `Niva.api.window.setCursorGrab` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察；补充 Windows 原生调用观察 |
...
| `Niva.api.windowExtra.setHasShadow` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setIsDocumentEdited` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.isDocumentEdited` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setAllowsAutomaticWindowTabbing` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.allowsAutomaticWindowTabbing` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setTabbingIdentifier` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.tabbingIdentifier` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setTrafficLightInset` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setActivationPolicyAtRuntime` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setDockVisibility` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
| `Niva.api.windowExtra.setBadgeLabel` | 未发现逐方法行为断言 | 未观察 | 未观察 | 增加逐方法行为测试；补充 macOS 原生调用观察 |
...
| Method | unit/JS behavior test pointer | macOS native observed | Windows native observed | remaining case needed |
|---|---|---|---|---|
| `Niva.require` | `bridge-api-overrides.test.mjs` — `Niva module registration, require, and import preserve module identity` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.registerModule` | `bridge-api-overrides.test.mjs` — `Niva module registration, require, and import preserve module identity` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.import` | `bridge-api-overrides.test.mjs` — `Niva module registration, require, and import preserve module identity` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.addEventListener` | `bridge-api-overrides.test.mjs` — `Niva event subscriptions match exact, namespace, and wildcard names` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.removeEventListener` | `bridge-api-overrides.test.mjs` — `Niva event subscriptions match exact, namespace, and wildcard names` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.removeAllEventListeners` | `bridge-api-overrides.test.mjs` — `Niva event subscriptions match exact, namespace, and wildcard names` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.call` | `bridge-api-overrides.test.mjs` — `Niva.call resolves successful replies and rejects native errors` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.stream` | `bridge-api-overrides.test.mjs` — `Niva.stream cancellation and streamSend use the call ID and binary frame` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
| `Niva.streamSend` | `bridge-api-overrides.test.mjs` — `Niva.stream cancellation and streamSend use the call ID and binary frame` | 未观察 | 未观察 | 补充独立行为用例及 macOS/Windows 真机观察。 |
