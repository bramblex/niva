...
Devtools 多目标构建 / CLI
  ...
  → 同一份项目资源 + 原样保留的 NodeCompat 配置
  → Windows PE 组装 / macOS app 组装与 ad hoc 签名
  → 资源读回或签名封装、归档
  → 每目标 staging 完成后发布到输出目录
```

每个目标独立成功或失败，已有文件拒绝覆盖。输出通过同文件系统硬链接原子发布；目标文件系统不支持硬链接时返回错误，不降级为不完整输出。资源文件按路径排序，显式 `niva.json` 放在数据首部。NodeCompat JavaScript 资源由 Niva 主程序在构建时嵌入；packager 只打包项目文件并原样保留 `niva.json`，由运行时按配置启用、关闭或筛选模块。开启 NodeCompat 时，用户资源不得占用内置资源命名空间 `__niva_compat/`；资源根内符号链接、路径越界和特殊文件也会被拒绝。
...
`.github/workflows/packager.yml` 分别使用 Windows x64、Mac arm64、Mac Intel runner 编译运行时和宿主 packager，再汇总三个运行时生成离线工具包。NodeCompat 资源随运行时构建压缩并嵌入，不再由 Devtools 或工具包生成流程单独 stage/extract。`scripts/create-packager-kit.py` 生成版本/哈希清单、文件校验和、依赖许可原文与精确版本源码链接；工具包保留 NodeCompat 的第三方许可说明。工作流上传构件，不自动发布正式版本。
...
历史候选 `228c9afc33fcfc46ed13cbb7e09685156c2e6ebc` 曾记录[常规 CI](https://github.com/bramblex/niva/actions/runs/35955733263)与[三宿主工具包工作流](https://github.com/bramblex/niva/actions/runs/35955733328)结果。该候选记录不代表当前集成工作区已通过远端 CI，也不代替下方保留的真机和下载验收项。

该历史候选记录了 Windows x64 静态 CRT 设置，以避免此前产物对 VC Runtime DLL 的依赖。报告列出的产物为：
...
该历史报告仍明确列出未完成项：Windows 连接变得不可用，未取得最终静态 CRT 产物的 GUI smoke 回报；此前动态 CRT 候选的真机启动结果不能作为最终静态 CRT 产物的启动证明。原生 Devtools 点击/文件夹对话框也因 Mac 锁屏未完成；已有浏览器 mock 交互验证。报告当时记为 PR 草稿、未合并 main。以上是指定 commit 的历史记录，不能替代当前代码的远端 CI 或全平台验收。

## 本次整合工作区验证（2026-09-24，macOS arm64）

- Rust `cargo fmt --all -- --check`、`cargo check --workspace`、`cargo clippy --workspace --all-targets` 与 `cargo test --workspace` 均通过；`niva-packager` 单测 15 项、集成测试 2 项通过。`cargo build --release -p niva -p niva-packager` 成功。
- Release `niva` 为 2,788,736 bytes，SHA-256 `139dd0305f17031bad69c11ba2dc000c82a5d831b36473e453de368079c6a719`，低于 3,300,000-byte 主程序门禁；独立 `niva-packager` 为 4,118,592 bytes，SHA-256 `5f5f13eebfd60759ab92094a1a845921a3a31e9f9f06b6f9eee2eada8584970b`，与运行时分开统计。
- 本机用 release packager 和临时 `macos-aarch64` 运行时清单打包后，`scripts/packager-smoke.py --artifact` 通过 ad-hoc 签名校验、ZIP 执行位检查和实际启动；stdio smoke 回报 OS、窗口标题、资源读取、文件读写及 `node:path` 检查成功。ZIP 为 1,654,787 bytes，SHA-256 `ce65d05ac708e7d3b9121cb8673142d3bff1fac82ff530834f61383e918a7c80`。
- `examples/node-compat-integration/run.py target/release/niva --packaged` 通过；native ready 与 packaged 结果均为 true，报告 179 项 API surface，脚本内有限集成检查全部成功。Devtools Vite/TypeScript 构建通过（输出提示有 chunk 大于 500 kB），Devtools 脚本测试 21/21、NodeCompat 测试 109/109、`python3 -m py_compile scripts/*.py` 均通过。
- `scripts/create-packager-kit.py` 的组装检查使用一个真实 macOS arm64 runtime 和两个明确标记为 fixture 的占位文件（Windows/Intel）；没有用这些占位文件测试运行时或应用。生成的工具包未包含外置 NodeCompat 资源目录，包含 NodeCompat 与 native dependency 许可文件，`SHA256SUMS` 中 1,196 项均匹配。
- Windows 全 workspace target check 因 `ring` 缺少 MSVC C 头文件失败；`cargo check -p niva --target x86_64-pc-windows-msvc` 通过。该交叉编译检查不等同 Windows 运行。
- 本记录是当前 macOS arm64 工作区与临时产物的实测；未验证当前提交的远端 CI、Windows 真机 GUI、Intel 真机运行、真实下载后的 Gatekeeper 首开或原生 Devtools 多目标界面操作。此前其他候选的 CI 和 Rosetta 记录不替代这些验收。
