...
- **Windows 真机**：确认 WebView2 Runtime 的实际 `http://niva.app` origin、同源/跨源 iframe 请求、WS `Origin`、CSP、NodeCompat、动态端口及导航时序。`cargo check --target x86_64-pc-windows-msvc` 已通过，但不能代替真机。
