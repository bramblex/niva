...
- [ ] **资源路径穿越 + HTTP 路径边界验收**：`FileSystemResource` 已加入解码后
  ...
  精确 origin 可读、缺失 token 返回 403、非匹配 Origin 无 ACAO；Windows 已用打包页
  验证有效/无效 token、错误 Origin 和普通静态路由 404，完整来源矩阵仍待验收。
...
## P0 —— 正确性缺口
- [x] **Windows 构建验证**：`cargo check --workspace`、测试和实际 `build_Windows.cmd`
  已在 Windows x86_64 真机通过；共享 release profile 使用 `panic=abort` 后 Windows 裸 Niva
  为 2,450,432 字节，Devtools exe 可启动，ZIP 完整性通过。详情与未覆盖范围见
  [验证记录](windows-validation-2026-09-23.md)。
...
- [ ] **共享 release panic 策略复验**：`[profile.release]` 现对 macOS/Windows 都使用 `panic=abort`；Windows 已重建并跑过 stdio、打包页和严格 CSP smoke。此前 macOS 双架构产物使用旧策略，须在 Mac 上重新构建并验收；macOS IPC 的 `catch_unwind` 在 abort release 中不再拦截 panic。
