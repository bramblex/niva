...
| `fs.cp` | 中 | 部分兼容 | Native递归复制 | JS callback/Promise适配 / 中 | WS → 不纳入IPC | Rust负责递归遍历、内容复制、覆盖、符号链接与主要选项；无filter时整棵树一次Native调用。提供filter时JS执行Node回调并遍历元数据，随后由Native逐项复制，文件内容不经JS或IPC。`COPYFILE_FICLONE_FORCE`当前明确返回ENOTSUP；OS特殊文件及错误文本仍按平台有差异。 | [fs.ts](../packages/runtime/src/runtime/fs.ts)；[fs.rs](../crates/niva/src/app/api/fs.rs) |
| `fs/promises.cp` | 中 | 部分兼容 | Native递归复制 | JS Promise/Node选项适配 / 中 | WS → 不纳入IPC | Rust负责递归遍历、内容复制、覆盖、符号链接与主要选项；无filter时整棵树一次Native调用。提供filter时JS执行Node回调并遍历元数据，随后由Native逐项复制，文件内容不经JS或IPC。`COPYFILE_FICLONE_FORCE`当前明确返回ENOTSUP；OS特殊文件及错误文本仍按平台有差异。 | [fs.ts](../packages/runtime/src/runtime/fs.ts)；[fs.rs](../crates/niva/src/app/api/fs.rs) |
...
| API | 频率 | 当前状态 | Native 能力 | 工作层 / 难度 | 本地 → 跨域目标 | 差距与依赖依据 | 源码 |
|---|---|---|---|---|---|---|---|
| `http.Server.listen` | 高 | 部分兼容 | Native TCP stream | JS HTTP/1.1服务端 / 中 | 本地WS服务→Native socket；不经IPC | `createServer().listen()`由JS解析请求、Native net提供TCP连接；无连接复用/upgrade。 | [http.ts](../packages/runtime/src/runtime/http.ts)；[net.ts](../packages/runtime/src/runtime/net.ts) |
| `http.createServer` | 高 | 部分兼容 | Native TCP stream | JS HTTP/1.1服务端 / 中 | 本地WS服务→Native socket；不经IPC | JS处理HTTP framing、headers、request/response流和trailers；Native ureq客户端不能替代服务端能力。 | [http.ts](../packages/runtime/src/runtime/http.ts)；[socket.rs](../crates/niva/src/app/api/socket.rs) |
| `http.get` | 高 | 部分兼容 | ureq HTTP client stream | JS Node对象适配 + Rust协议 / 中 | 受信本地WS→Native ureq；IPC仅`requestText` | 调用`request().end()`并返回ClientRequest；body按块上传、响应按块读取。无Agent连接池/原生Socket/upgrade；默认总时限10秒、可设至30秒；reason phrase使用规范值。 | [http.ts](../packages/runtime/src/runtime/http.ts)；[http.rs](../crates/niva/src/app/api/http.rs) |
| `http.request` | 高 | 部分兼容 | ureq HTTP client stream | JS Node对象适配 + Rust协议 / 中 | 受信本地WS→Native ureq；IPC仅`requestText` | ClientRequest/IncomingMessage保留Readable/Writable契约；受控16KiB上传块、逐块上传确认和下载背压。无Agent复用、底层net.Socket、custom connection、upgrade；Native总超时与Node idle timeout不完全一致。 | [http.ts](../packages/runtime/src/runtime/http.ts)；[http.rs](../crates/niva/src/app/api/http.rs) |
...
| API | 频率 | 当前状态 | Native 能力 | 工作层 / 难度 | 本地 → 跨域目标 | 差距与依赖依据 | 源码 |
|---|---|---|---|---|---|---|---|
| `https.get` | 高 | 部分兼容 | ureq HTTP/TLS client stream | JS Node对象适配 + Rust协议 / 中 | 受信本地WS→Native ureq；IPC仅`requestText` | 调用`request().end()`返回ClientRequest；平台系统根、SNI、主机名验证由Rust保留。无Agent连接池、原生Socket和upgrade。 | [https.ts](../packages/runtime/src/https.ts)；[http.ts](../packages/runtime/src/runtime/http.ts)；[http.rs](../crates/niva/src/app/api/http.rs) |
| `https.request` | 高 | 部分兼容 | ureq HTTP/TLS client stream | JS Node对象适配 + Rust协议 / 中 | 受信本地WS→Native ureq；IPC仅`requestText` | 流式上传/下载复用ureq TLS验证；无Agent复用/底层net.Socket/custom connection/upgrade；默认总时限10秒、最多30秒，Native规范statusMessage不保留定制reason。 | [https.ts](../packages/runtime/src/https.ts)；[http.ts](../packages/runtime/src/runtime/http.ts)；[http.rs](../crates/niva/src/app/api/http.rs) |
| `https.createServer` | 中 | 部分兼容 | Native TLS stream | JS HTTP/1.1服务端 / 中 | 本地WS服务→Native TLS；不经IPC | JS保留HTTP服务端协议逻辑，Native TLS提供服务端socket。 | [https.ts](../packages/runtime/src/https.ts)；[http.ts](../packages/runtime/src/runtime/http.ts)；[tls.ts](../packages/runtime/src/runtime/tls.ts) |
